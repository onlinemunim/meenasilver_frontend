...
@Component({
  ...
  templateUrl: './sell-details-lot.component.html',
  styleUrls: ['./sell-details-lot.component.css'],
  providers: [DatePipe]
})
export class SellDetailsLotComponent implements OnInit {
  ...
  getProductCodeData(itemCode:any,quantity:any,weight:any,finalValue:any){
    ...
    this.readyProductService.getReadyProducts(params).subscribe((response:any)=>{
      ...
      this.sellProductForm.patchValue({
        ...
        stout_total_wt: weight,
        stout_unit_price : finalValue,
      });
      this.getRateListGeneratorData(product.rproduct_categories?.name,weight,finalValue)
    })
  }

  getRateListGeneratorData(category: any, net_weight: any,mrp:any) {
    this.isLoading = true;
    let params = new HttpParams().set('category', category);

    this.rateListGeneratorService.getRateList(params).subscribe((response: any) => {
      this.rateListData = response.data;
      console.log("rate list data is", this.rateListData);

      if (!this.rateListData?.length) return;

      if (this.rateListData[0].rlg_unit === "By Silver value") {
        this.sellProductForm.patchValue({
          stout_unit_price: '',
          stout_per_gram_labour: '',
          stout_weight_range: '',
          stout_purity: this.rateListData[0].rlg_purity,
          stout_final_amount: '',
          stout_fine_weight: ((net_weight * this.rateListData[0].rlg_purity) / 100).toFixed(2)
        });
        this.isLoading = false;
      } else if (this.rateListData[0].rlg_unit === "By MRP") {
        this.isLoading = false;
        this.sellProductForm.patchValue({
          stout_net_weight: '',
          stout_per_gram_labour: '',
        ...
        this.sellProductForm.get('stout_mrp_percent')?.valueChanges.subscribe(percent => {
        const p = Number(percent);
        if (!isNaN(p)) {
          this.sellProductForm.patchValue({
            stout_final_amount: ((mrp * p) / 100).toFixed(2)
          }, { emitEvent: false });
        } else {
          this.sellProductForm.patchValue({ stout_final_amount: '' }, { emitEvent: false });
        }
  });

      } else if (this.rateListData[0].rlg_unit === "By Gram") {
        const wt = Number(net_weight);

        let matchedRange = this.rateListData.find((rate: any) => {
          const min = rate.rlg_min_weight_range ? Number(rate.rlg_min_weight_range) : null;
          const max = rate.rlg_max_weight_range ? Number(rate.rlg_max_weight_range) : null;

          // Case 1: inside range
          if (min !== null && max !== null && wt >= min && wt <= max) {
            return true;
          }

          // Case 2: below min and rlg_below = Yes
          if (min !== null && wt < min && rate.rlg_below === "Yes") {
            return true;
          }

          // Case 3: above max (or no max) and rlg_above = Yes
          if ((max === null || max === 0 || wt > max) && rate.rlg_above === "Yes") {
            return true;
          }
        ...
        if (matchedRange) {
          this.sellProductForm.patchValue({
            stout_net_weight: wt,
            stout_fine_weight: wt,
            stout_unit_price: '',
            stout_weight_range: `${matchedRange.rlg_min_weight_range || ''} - ${matchedRange.rlg_max_weight_range || ''}`,
            stout_per_gram_labour: matchedRange.rlg_per_gm_labour,
            stout_final_amount: wt * Number(matchedRange.rlg_per_gm_labour)
          });
          this.isLoading = false;
        } else {
          this.sellProductForm.patchValue({
