...
@Component({
  selector: 'app-add-wholesale-stock-b3',
  standalone: true,
  imports: [
  ...
  templateUrl: './add-wholesale-stock-b3.component.html',
  styleUrls: ['./add-wholesale-stock-b3.component.css']
})
export class AddWholesaleStockB3Component implements OnInit, AfterViewInit {
  ...
  // Dynamic stone forms with all properties
  stoneForms: any[] = [
    {
      id: 1, stoneId: '', stoneName: '', certificate: '', laboratory: '', clarity: '', color: '', size: '', shape: '', qty: '', gsWt: '', unit: '', purRate: '', sellRate: ''
    }
  ];

  addStoneForm(): void {
    const newId = this.stoneForms.length > 0
      ? Math.max(...this.stoneForms.map(f => f.id)) + 1
      : 1;
    this.stoneForms.push({
      id: newId, stoneId: '', stoneName: '', certificate: '', laboratory: '', clarity: '', color: '', size: '', shape: '', qty: '', gsWt: '', unit: '', purRate: '', sellRate: ''
    });
  }

  removeStoneForm(formId: number): void {
    this.stoneForms = this.stoneForms.filter(form => form.id !== formId);
  }
}
