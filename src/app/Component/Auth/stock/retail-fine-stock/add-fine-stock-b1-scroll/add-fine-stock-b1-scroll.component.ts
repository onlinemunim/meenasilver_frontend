...
@Component({
  ...
  styleUrls: ['./add-fine-stock-b1-scroll.component.css'],
})
export class AddFineStockB1ScrollComponent implements OnInit, AfterViewInit {
    ...
    ['st_bill_date', 'st_mfd_date', 'st_exp_date'].forEach((field) => {
      this.fineStockForm.get(field)?.valueChanges.subscribe((val) => {
        if (typeof val === 'string') {
          ...
          if (formatted !== val) {
            this.fineStockForm.get(field)?.setValue(formatted, {
              emitEvent: false,
            });
          }
        }
  ...
  onDateInput(field: 'mfdDate' | 'billDate' | 'expDate', event: Event) {
    ...
    if (formatted !== val) {
      ...
      this.fineStockForm.get(controlMap[field])?.setValue(formatted, {
        emitEvent: false,
      });
    }
  }
  ...
  onSubmit() {
    ...
    ['st_bill_date', 'st_mfd_date', 'st_exp_date'].forEach((field) => {
      ...
      if (val instanceof Date) {
        this.fineStockForm
          .get(field)
          ?.setValue(val.toISOString().split('T')[0]);
    ...
    this.stockService.createStockEntry(this.fineStockForm.value).subscribe(
      (res) => {
        this.notificationService.showSuccess(
          'Fine Stock Entry Created Successfully!',
          'Success'
        );
        this.clearForm();
      },
      (error) => {
        this.notificationService.showError(
          'Failed to create Fine Stock Entry',
          'Error'
        );
      }
    );
