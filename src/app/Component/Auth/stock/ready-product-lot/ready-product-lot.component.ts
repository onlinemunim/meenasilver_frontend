...
@Component({
  selector: 'app-ready-product-lot',
  standalone: true,
  imports: [CustomSelectComponent, NgIf, ReactiveFormsModule, NgClass, NgFor,RouterModule],
  templateUrl: './ready-product-lot.component.html',
  styleUrl: './ready-product-lot.component.css',
  providers: [DatePipe]
})
export class ReadyProductLotComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // Removed duplicate patchMetalCode function

  // Removed duplicate clearMetalCode() implementation
  ...
  initReadyProduct(){
    ...
    this.readyProductForm = this.fb.group({
      ...
      rproduct_net_weight: [''],
      rproduct_total_panni_weight: [''],
      rproduct__per_gm_cost:[''],
      rproduct_type:'lot-wise',
  ...
  createReadyProduct() {
    ...
    this.readyProductService.createReadyProduct(formData).subscribe(
      (response: any) => {
        ...
        console.log("submitted data is",this.readyProductData);
        ...
        if (response?.data?.length > 0) {
          ...
          if (barcodePrefix) {
            this.customizeService.getNextBarcode(barcodePrefix).subscribe({
              next: (res: any) => {
                if (res?.code) {
                  console.log("BARCODE IS",res.code);

                  this.readyProductForm.patchValue({ rproduct_barcode: res.code });
                } else {
                  console.warn('Backend did not return a barcode.');
                }
              },
              error: (err) =>
                console.error('Error fetching the next barcode:', err),
            });
          } else {
            console.warn(
              'Barcode prefix ("customize_barcode_prefix") is not set in settings.'
            );
          }
        } else {
          console.warn('Could not find any customize settings data.');
        }
      },
      error: (err) => console.error('Error fetching customize settings:', err),
    });
  }
  ...
  getProductCodes() {
    this.rawMetalService.getReadyProductByType('lot-wise').subscribe((response: any) => {
  ...
  onSelectProductCode(code: string) {
    this.isManuallySelectingCode = true;
    ...
    this.rawMetalService.searchByStCode(code).subscribe({
      next: (response: any) => {
        ...
        if (dataArray.length >= 1) {
          ...
          if (rawMetalData) {
            this.readyProductForm.patchValue({
              rproduct_name: rawMetalData.rproduct_name,
              rproduct_categories: rawMetalData.rproduct_categories?.name,
              rproduct_subcategories: rawMetalData.rproduct_subcategories,
              rproduct_stonetype: rawMetalData.rproduct_stonetype,
              rproduct_stone_color: rawMetalData.rproduct_stone_color,
              rproduct_size: rawMetalData.rproduct_size,
              rproduct_carat: rawMetalData.rproduct_carat,
              rproduct_total_panni_weight: rawMetalData.rproduct_total_panni_weight,
              rproduct__per_gm_cost: rawMetalData.rproduct__per_gm_cost,
              rproduct_net_weight: rawMetalData.rproduct_net_weight,
              rproduct_supplier_name: rawMetalData.rproduct_supplier_name,
              rproduct_supplier_code: rawMetalData.rproduct_supplier_code,
              rproduct_polishtype: rawMetalData.rproduct_polishtype,
              rproduct_supplier_product_code: rawMetalData.rproduct_supplier_product_code,
              rproduct_per_carat_price: rawMetalData.rproduct_per_carat_price,
              rproduct_total_carat_price: rawMetalData.rproduct_total_carat_price,
              rproduct_stone_price: rawMetalData.rproduct_stone_price,
              rproduct_silver_rate_per_gm: rawMetalData.rproduct_silver_rate_per_gm,
              rproduct_per_gram_labour: rawMetalData.rproduct_per_gram_labour,
              rproduct_shipping_per_gm: rawMetalData.rproduct_shipping_per_gm,
              rproduct_other_expense_per_gm: rawMetalData.rproduct_other_expense_per_gm,
              rproduct_total_wt: rawMetalData.rproduct_total_wt,
              rproduct_quantity: rawMetalData.rproduct_quantity,
              rproduct_unit: rawMetalData.rproduct_unit || 'GM(Gram)',
          }, { emitEvent: false });
          }
        } else {
  ...
  patchProductCodeRelatedDataWithGeneralDetails(code: string) {
    ...
    if (productData) {
      this.readyProductForm.patchValue({
        ...
        rproduct_categories: productData.product_category?.name,
  ...
  selectPackagingStack(item: any) {
    ...
    // Fetch price data and patch purchase price
  ...
  getPriceDataOfPackagingStack(id: any) {
    this.stockPriceService.getPriceDataByProductId(id).subscribe((res: any) => {
      const purchasePrice = res.data[0]?.purchase_price || 0;
      this.readyProductForm.patchValue({
        rproduct_packaging_cost: purchasePrice
      });
    });
  }
  ...
  getReadyProductList() {
    const params = new HttpParams()
      ...
      .set('ready_product_type', 'lot-wise');
